...
magnetic flux density ($B$) and the magnetising field $H$, known as the \(B-H\) curve. By altering the transformer's core, its response can be tuned. For ferromagnetic materials, the \(B-H\) curve is non-linear and exhibits 
...
$\\$
The most efficient transformers are made by having a high relative permeability ($\mu_r$) and low energy 
loss per cycle. This experiment aims to quantify the usability of different materials as transformer cores 
by measuring the energy loss per cycle per unit volume (given by the area of a hysteresis loop) and the maximum 
