...
Table~\ref{tab:summary-metrics} collects each sample’s key \(B(H)\) metrics, peak and minimum \(\mu_{\mathrm{r}}\), and power loss per unit area so the datasets can be compared directly.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccccc}
    \toprule
    Sample & \(B\) range (\si{\tesla}) & \(H\) range (\si{\ampere\metre^{-1}}) & Max \(\mu_{\mathrm{r}}\) & Min \(\mu_{\mathrm{r}}\) & Power loss per unit area (\si{\watt\per\metre\cubed}) \\
    \midrule
    ...
    CuNi (\SI{40}{\celsius}) & 0.0954 & \(5.56\times 10^{4}\) & 1.33 & 1.33 & negligible \\
    \bottomrule
  \end{tabular}
\end{table}
$\\$
